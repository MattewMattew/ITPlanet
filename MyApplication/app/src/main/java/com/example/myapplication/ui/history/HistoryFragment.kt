...
class HistoryFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        val recycler: RecyclerView = view.findViewById(R.id.recyclerHistory)
        recycler.layoutManager = LinearLayoutManager(this.context)
        val fileName = activity?.cacheDir?.absolutePath+"/History.json"
        if (File(fileName).canRead()){
            val listUp = readJSONfromFileUpdate(fileName)
            recycler.adapter = MyRecyclerAdapter(listUp,this@HistoryFragment, clickListener = {listen(it)})
        }
        else{
